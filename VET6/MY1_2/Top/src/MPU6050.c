...
#define MOTION          (0)
#define NO_MOTION       (1)
#define DEFAULT_MPU_HZ  (200)    //更新MPU数据的频率（200HZ  -->  5ms）
#define FLASH_SIZE      (512)    //闪存大少
#define FLASH_MEM_START ((void*)0x1800)
	
#define q30  1073741824.0f       // 四元数 - 转化数据的重要参数


short gyro[3], accel[3], sensors;            //存放陀螺仪、加速度、温度的数据
float Pitch,Roll,Yaw;                        //Pitch 是绕 x 轴旋转的角度、Roll 是绕 y 轴旋转的角度、Yaw 是绕 z 轴旋转的角度
float q0=1.0f,q1=0.0f,q2=0.0f,q3=0.0f;       //定义四元数
...
void MPU6050_initialize(void) {
     MPU6050_setClockSource(MPU6050_CLOCK_PLL_YGYRO);     //设置时钟   // 初始化使用的内部时钟
...
入口参数：无
返回  值：无
注意  必须开串口重定向 才可以使用  printf,要么屏蔽它
**************************************************************************/
void DMP_Init(void)
{ 
   u8 temp[1]={0};
...
{	
	  unsigned long sensor_timestamp;
		...
		long quat[4];
				...
				dmp_read_fifo(gyro, accel, quat, &sensor_timestamp, &sensors, &more);		  //quat ：四元数
				...
				{    
					 ...
					 q2=quat[2] / q30;
					 q3=quat[3] / q30;
					 Pitch = asin(-2 * q1 * q3 + 2 * q0* q2)* 57.3; 	   // 绕 x 轴的角度
					 Roll  = atan2(2 * q2 * q3 + 2 * q0 * q1, -2 * q1 * q1 - 2 * q2* q2 + 1)* 57.3; // 绕 y 轴的角度
